...
#ifndef LINT
static char *const _cddbd_c_ident_ = "@(#)$Id: cddbd.c,v 1.51 2006/04/21 19:03:48 megari Exp $";
#endif
...
void do_cddb_write(arg_t *args)
{
	...
	case DE_NO_ERROR:
	  ...
	  if(!strcmp(postdir, cddbdir)) {
	    cddbd_snprintf(buf, sizeof(buf), "%08x", discid);
	    (void)db_match_chk(db, category, buf, &ncategory, 0);
	  }
	  else
	    ncategory = category;
