...
#ifndef LINT
static char *const _db_c_ident_ = "@(#)$Id: db.c,v 1.70 2006/04/21 19:13:50 megari Exp $";
#endif
...
int db_match(db_t *, db_t *, char *, char *);
int db_classify(char *, int *, char **);
int db_col(db_t *, db_t *);
int db_cmp(db_t *, db_t *, char *, char *);
int db_cmp_rev(db_t *, db_t *);
...
void cddbd_update(void)
{
	...
	for(i = 0; categlist[i] != 0; i++) {
		...
		while((dp = readdir(dirp)) != NULL) {
			...
			if(copy || merge) {
				...
				ret = db_match_chk(db, categlist[i],
				    dp->d_name, &ncategory, 0);

				if(!cddbd_write_ulist(ncategory,
				    dp->d_name)) {
					cddbd_log(LOG_ERR | LOG_UPDATE,
	...
	cddbd_log(LOG_INFO,
	    "Updated %d, %d recategorized, %d duplicate, %d bad, %d unreadable.",
		  updated, recat, duped, bad, noread);
...
cddbd_check_db(int check_level, int fix_level)
{
	...
	for(i = 0; categlist[i] != 0; i++) {
		...
		while((dp = readdir(dirp)) != NULL) {
			...
			if(check_level >= CL_MATCH)
				recat += db_match_chk(db, categlist[i],
				    dp->d_name, 0, flags);
	...
	cddbd_log(LOG_INFO,
	    "Found %d normal files, %d cross-categorized, %d links, %d invalid, %d unreadable.",
		  count, recat, links, bad, noread);
	cddbd_log(LOG_INFO, "Done checking the database.");
}
...
int db_post(db_t *db, char *dir, unsigned int discid, char *errstr)
{
	...
	/*if(!strcmp(postdir, cddbdir)) */ /* I think this is wrong. -megari */
...
int
db_cmp(db_t *db1, db_t *db2, char *tit1, char *tit2)
{
	int ret;

	/* Make sure they're the same CD. Give db2 precedence. */
	if(db_match(db1, db2, tit1, tit2) != DM_MATCH)
		return -1;

	/* Compare revisions. */
	ret = db_cmp_rev(db1, db2);

	/* Take the one with the longest name, if identical. */
	if(ret == 0 && tit1 != 0 && tit2 != 0) {
		if(strlen(tit1) > strlen(tit2))
			ret = 1;
		else
			ret = -1;
	}
...
db_unlink(char *dir, char *name)
{
	...
	if(stat(buf, &sbuf) || (fp = fopen(buf, "r")) == NULL) {
		unlink(buf);
		return;
	}
	...
	if(db == 0) {
		...
		for(list_rewind(lh), list_forw(lh); !list_rewound(lh);
			...
			cddbd_snprintf(buf, sizeof(buf), "%s/%08x",
			    dir, discid);
			/* Only unlink if this link is to the same file. */
			if(stat(buf, &sbuf2) == 0 &&
			   (sbuf.st_ino == sbuf2.st_ino)) {
			  unlink(buf);

			  cddbd_log(LOG_INFO,
				    "Unlink: %s %08x",
				    cddbd_basename(dir), discid);
			}
		}
...
db_link(db_t *db, char *dir, char *name, int fix)
{
	...
	for(list_rewind(lh), list_forw(lh); !list_rewound(lh); list_forw(lh)) {
		...
		if(stat(dblink, &sbuf2) == 0) {
			/* Uh-oh, the two are different files. */
			if(sbuf.st_ino != sbuf2.st_ino) {
				if(fix) {
					/* If the two are the same, merge. */
					if(db_vfy_link(db, dbname, dblink)) {
						lh = db->db_phase[DP_DISCID];
						list_rewind(lh);
						continue;
					}
...
int
db_vfy_link(db_t *db1, char *dbname, char *dblink)
#ifdef DB_WINDOWS_FORMAT
{
  return 1; /* XXX I wonder if this should be
	       0 or 1 - anyway, there is no
...
#else
{
  ...
  db_t dbt;
  db_t *db2;
  char tit1[CDDBBUFSIZ];
  char tit2[CDDBBUFSIZ];
  char errstr[CDDBBUFSIZ];

  /* Read the link and nuke it if it's the same. */
  if((fp = fopen(dblink, "r")) == NULL)
    return 0;
  ...
  /* If db2 is preferable, swap it with db1. */
  if(db_cmp(db1, db2, tit1, tit2) < 0) {
    dbt = *db1;
    *db1 = *db2;
    *db2 = dbt;
  }

  (void)db_merge(db1, db2);
  db_free(db2);

  if((fp = fopen(dbname, "w")) == NULL || db_write(fp, db1, MAX_PROTO_LEVEL) == 0) {
    cddbd_log(LOG_ERR | LOG_UPDATE,
	      "Can't write CDDB file: %s", dbname);
...
/* As this was copied directly from 1.4b48, it might need some work */

int
db_match_chk(db_t *db1, char *categ, char *discid, char **ncateg, int flags) {
#ifdef DB_WINDOWS_FORMAT
	if(ncateg != 0)
		*ncateg = categ;

	return 0; /* TODO: Do something meaningful */
#else
	int i;
	int cnt;
	int ret;
	...
	char buf2[CDDBBUFSIZ];
	char dir1[CDDBBUFSIZ];
	char dir2[CDDBBUFSIZ];
	char file1[CDDBBUFSIZ];
	char file2[CDDBBUFSIZ];
	char errstr[CDDBBUFSIZ];
	lhead_t *lh;
	...
	for(i = 0; categlist[i] != 0; i++) {
		...
		cddbd_snprintf(dir1, sizeof(dir1), "%s/%s", cddbdir, categ);
		cddbd_snprintf(dir2, sizeof(dir2), "%s/%s", cddbdir,
		    categlist[i]);
		cddbd_snprintf(file1, sizeof(file1), "%s/%s", dir1, discid);
		cddbd_snprintf(file2, sizeof(file2), "%s/%s", dir2, discid);
		...
		/* Doesn't matter why we can't read it, just continue. */
		if(db2 == 0)
			continue;

		/* If they don't match, continue. */
		if((ret = db_match(db1, db2, buf1, buf2)) != DM_MATCH) {
			if(verbose && (ret == DM_NOTEXT)) {
				cddbd_log(LOG_INFO, "Possible duplicate (weak "
				    "match): %s: \"%s\" and %s \"%s\"\n",
				    file1, buf1, file2, buf2);
			}
		...
		if(flags & DF_NOFIX) {
			cddbd_log(LOG_INFO, "Possible duplicate: "
			    "%s: \"%s\" and %s \"%s\"\n", file1, buf1,
			    file2, buf2);
		...
		/* Find the best one, and give db1 precedence if equal. */
		if(db_cmp(db1, db2, buf1, buf2) < 0) {
			/* db2 is preferable, so swap it with db1. */
			dbt = *db1;
			*db1 = *db2;
			*db2 = dbt;
		...
		cddbd_log(LOG_INFO,
		    "Merging duplicate: %s: \"%s\" into %s: \"%s\"\n",
		    filef, buff, filet, buft);

		p = discid;
		if(db_parse_discid(&p, &tmpid) != 0) {
		  cddbd_log(LOG_ERR, "Unexpected character in discid: 0x%02x\n", *p);
		  lh = db1->db_phase[DP_DISCID];
		  list_rewind(lh);
		  list_forw(lh);
		  tmpid = ptr_to_uint32(list_cur(lh)->l_data);
		}

		(void)db_merge(db1, db2);
		(void)db_post(db1, dirt, tmpid, errstr);

		if(db_errno == DE_NO_ERROR) {
			cnt++;
			db_unlink(dirf, discid);
		}
		else {
			cddbd_log(LOG_ERR | LOG_UPDATE,
			    "Failed to write DB entry %s: %s", filet, errstr);
		}
...
int
db_match(db_t *db1, db_t *db2, char *tit1, char *tit2)
{
	char *p1;
	char *p2;

	/* Compare track timings. */
	if(db1->db_trks != db2->db_trks ||
	    !is_fuzzy_match(db1->db_offset, db2->db_offset,
	    db1->db_disclen, db2->db_disclen, db1->db_trks))
		return DM_NOFUZ;

	if(tit1 != 0 && tit2 != 0) {
		/* Compare the disc titles. */
		db_strcpy(db1, DP_DTITLE, 0, tit1, CDDBBUFSIZ);
		db_strcpy(db2, DP_DTITLE, 0, tit2, CDDBBUFSIZ);

		if((p1 = text_reduce(tit1)) == 0 ||
		    (p2 = text_reduce(tit2)) == 0) {
			if(p1 != 0)
				free(p1);
		...
		/* Is the text similar enough to be considered identical? */
		if(text_correlate(p1, p2) < TEXT_THRESHOLD) {
			free(p1);
			free(p2);
